...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $reqOk) {
    ...
    if (!$host || !$name) {
        ...
        try {
            ...
            // 2b. Persist DB credentials so the app (db()) connects with the same account.
            //     Without this, config/database.php falls back to root and fails.
            $cfgFile = __DIR__ . '/config/db.config.php';
            $cfgBody = "<?php\n// Generated by install.php — DB credentials for db(). Do not commit.\nreturn "
                . var_export(['host' => $host, 'name' => $name, 'user' => $user, 'pass' => $pass], true)
                . ";\n";
            if (@file_put_contents($cfgFile, $cfgBody) !== false) {
                $results[] = ['ok', "บันทึกการตั้งค่าฐานข้อมูลไปที่ config/db.config.php แล้ว"];
            } else {
                $results[] = ['warn', "บันทึก config/db.config.php ไม่สำเร็จ — ตรวจสอบสิทธิ์เขียนโฟลเดอร์ config/ (แอปอาจเชื่อมต่อ DB ไม่ได้)"];
            }
